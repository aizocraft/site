...
function productRoutes(productModel) {
    ...
    const generateSKU = async (category) => {
        ...
        const existingProducts = await Product_1.default.find({
    ...
    router.get('/', async (req, res) => {
        try {
            const { category, q: search, sort = 'createdAt', order = 'desc', page = '1', limit = '12', featured, minPrice, maxPrice, minRating, tags, supplier, minProfitMargin, lowStock } = req.query;
            // Build filter query
            const query = {};
            if (category)
     ...
     */
    router.post('/upload-images', auth_1.default, upload.array('images', 6), async (req, res) => {
        var _a;
        try {
     ...
     */
    router.put('/slug/:slug', auth_1.default, async (req, res) => {
        var _a;
        try {
            const role = (_a = req.user) === null || _a === void 0 ? void 0 : _a.role;
            if (role !== 'admin' && role !== 'sales') {
            ...
            const product = await Product_1.default.findOneAndUpdate({ slug }, updateData, { new: true, runValidators: true });
            if (!product) {
                return res.status(404).json({ error: 'Product not found' });
            }
            res.json(product);
     ...
     */
    router.post('/:id/upload-images', auth_1.default, upload.array('images', 6), async (req, res) => {
        var _a;
        try {
            const role = (_a = req.user) === null || _a === void 0 ? void 0 : _a.role;
            if (role !== 'admin' && role !== 'sales') {
     ...
     */
    router.delete('/:id/images/:index', auth_1.default, async (req, res) => {
        try {
            if (!isAdmin(req)) {
                return res.status(403).json({ error: 'Admin access required' });
            }
                    ...
                    const notificationPromises = adminUsers.map(admin => {
                        ...
                        return (0, notification_service_1.createNotification)({
                            ...
                            metadata: {
                                ...
                                deletedBy: ((_a = req.user) === null || _a === void 0 ? void 0 : _a.email) || ((_b = req.user) === null || _b === void 0 ? void 0 : _b.name) || 'Admin',
