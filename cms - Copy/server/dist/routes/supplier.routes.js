...
router.get('/', auth_1.default, async (req, res) => {
    try {
        const user = req.user;
        if ((user === null || user === void 0 ? void 0 : user.role) !== 'admin' && (user === null || user === void 0 ? void 0 : user.role) !== 'sales') {
...
router.post('/', auth_1.default, async (req, res) => {
    try {
        ...
        await notifyAdmins('🏭 New Supplier Added', `${req.user.email || req.user.name} added a new supplier: "${supplier.name}"`, `/dashboard/suppliers/${supplier._id}`, {
...
router.put('/:id', auth_1.default, async (req, res) => {
    try {
        ...
        const existingSupplier = await Supplier_1.default.findById(req.params.id);
        if (!existingSupplier) {
            return res.status(404).json({ error: 'Supplier not found' });
        }
        ...
        if (changes.length > 0) {
            await notifyAdmins('✏️ Supplier Updated', `${req.user.email || req.user.name} updated supplier "${supplier.name}": ${changes.join(', ')}`, `/dashboard/suppliers/${supplier._id}`, {
...
router.delete('/:id', auth_1.default, async (req, res) => {
    try {
        ...
        if (productCount > 0) {
            ...
            const products = await Product_1.default.find({ supplier: req.params.id }).select('name sku').limit(10);
        ...
        await notifyAdmins('🗑️ Supplier Deleted', `${req.user.email || req.user.name} deleted supplier "${supplierName}"`, '/dashboard/suppliers', {
...
router.patch('/:id/status', auth_1.default, async (req, res) => {
    try {
        ...
        await notifyAdmins(`${statusIcon} Supplier Status Changed`, `${req.user.email || req.user.name} changed supplier "${supplier.name}" status from ${oldStatus} to ${status}`, `/dashboard/suppliers/${supplier._id}`, {
...
router.get('/stats/summary', auth_1.default, async (req, res) => {
    try {
        ...
        const recentSuppliers = await Supplier_1.default.countDocuments({
...
router.get('/:id/products', auth_1.default, async (req, res) => {
    try {
        ...
        const [products, total] = await Promise.all([
            Product_1.default.find({ supplier: supplier._id })
                .select('name sku price buyingPrice stock status images')
                .sort(sort)
                .skip(skip)
                .limit(limitNum)
                .lean(),
            Product_1.default.countDocuments({ supplier: supplier._id })
        ]);
...
router.post('/:id/record-purchase', auth_1.default, async (req, res) => {
    try {
        if (!isAdmin(req)) {
            return res.status(403).json({ error: 'Admin access required' });
        }
        ...
        const purchaseHistory = supplier.get('purchaseHistory') || [];
        ...
        if (amount >= 100000) {
            await notifyAdmins('💰 Large Purchase Recorded', `${req.user.email || req.user.name} recorded a purchase of KES ${amount.toLocaleString()} from supplier "${supplier.name}"`, `/dashboard/suppliers/${supplier._id}`, {
                action: 'record_purchase',
...
router.get('/top', auth_1.default, async (req, res) => {
    var _a;
    try {
        if (!isAdmin(req)) {
            return res.status(403).json({ error: 'Admin access required' });
        ...
        const suppliers = await Supplier_1.default.find({ status: 'active' })
            .sort({ totalPurchases: -1 })
            .limit(parseInt(limit))
            .select('name totalPurchases lastPurchaseDate email phone')
            .lean();
        const totalPurchaseVolume = await Supplier_1.default.aggregate([
            { $group: { _id: null, total: { $sum: '$totalPurchases' } } }
        ]);
        res.json({
            suppliers,
            totalPurchaseVolume: ((_a = totalPurchaseVolume[0]) === null || _a === void 0 ? void 0 : _a.total) || 0,
...
router.get('/:id/purchase-history', auth_1.default, async (req, res) => {
    try {
        if (!isAdmin(req)) {
            return res.status(403).json({ error: 'Admin access required' });
        }
...
router.get('/:id/analytics', auth_1.default, async (req, res) => {
    try {
        if (!isAdmin(req)) {
            return res.status(403).json({ error: 'Admin access required' });
        }
...
router.post('/bulk', auth_1.default, async (req, res) => {
    try {
        ...
        if (createdSuppliers.length > 0) {
            await notifyAdmins('📦 Bulk Suppliers Created', `${req.user.email || req.user.name} created ${createdSuppliers.length} suppliers via bulk import`, '/dashboard/suppliers', {
...
router.patch('/bulk/status', auth_1.default, async (req, res) => {
    try {
        if (!isAdmin(req)) {
            return res.status(403).json({ error: 'Admin access required' });
        }
        ...
        const result = await Supplier_1.default.updateMany({ _id: { $in: supplierIds } }, { status });
        await notifyAdmins(`🔄 Bulk Supplier Status Updated to ${status}`, `${req.user.email || req.user.name} updated ${result.modifiedCount} suppliers to ${status}`, '/dashboard/suppliers', {
