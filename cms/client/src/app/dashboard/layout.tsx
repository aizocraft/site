...
import { 
  ...
  Receipt, MessageSquare, Mail, Building2
} from 'lucide-react'
...
export default function DashboardLayout({
  ...
  const { user, isAdmin, isEngineer, loading: authLoading } = useAuth()
...
const getActivePageFromPath = (path: string) => {
    ...
    if (path.startsWith('/dashboard/construction')) return 'construction'
  ...
  const [activePage, setActivePage] = useState<'overview'|'construction'|'products'|'sales'|'shipping'|'users'|'emails'|'submissions'|'reviews'|'auditlog'|'notifications'|'settings'>(
  ...
  useEffect(() => {
    ...
    if ((!user || !canAccessDashboard) && !hasRedirected.current && pathname !== '/orders') {
  ...
  }, [user, canAccessDashboard, authLoading, router, pathname])
