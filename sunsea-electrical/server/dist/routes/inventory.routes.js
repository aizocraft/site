...
const isSales = (req) => { var _a; return ((_a = req.user) === null || _a === void 0 ? void 0 : _a.role) === 'sales'; };
const isAdminOrSales = (req) => isAdmin(req) || isSales(req);
...
const generateSKU = async (category, existingSkus = []) => {
...
const generateUniqueSlug = async (name, existingSlugs) => {
...
const validateProductData = (data) => {
    ...
    if (data.buyingPrice && isNaN(parseFloat(data.buyingPrice)))
...
const parseCSVBuffer = (buffer) => {
    return new Promise((resolve, reject) => {
        const results = [];
        const readable = stream_1.Readable.from(buffer.toString());
        readable
...
const checkLowStockAndNotify = async (product, oldStock, newStock) => {
    const threshold = 10;
    ...
    try {
        ...
        const exportData = products.map(product => {
            ...
            return ({
                ...
                'Supplier Email': ((_a = product.supplier) === null || _a === void 0 ? void 0 : _a.email) || '',
                'Supplier Phone': ((_b = product.supplier) === null || _b === void 0 ? void 0 : _b.phone) || '',
                ...
                'Image URLs': ((_c = product.images) === null || _c === void 0 ? void 0 : _c.map((img) => img.url).filter(Boolean).join('; ')) || ''
            });
        });
        ...
        try {
            const json2csvParser = new json2csv_1.Parser({ fields: Object.keys(exportData[0]) });
...
router.get('/export/template', auth_1.default, async (req, res) => {
    try {
        if (!isAdminOrSales(req)) {
            return res.status(403).json({ error: 'Admin or sales access required' });
        }
        ...
        const json2csvParser = new json2csv_1.Parser({ fields: Object.keys(template[0]) });
...
router.post('/import', auth_1.default, upload.single('file'), async (req, res) => {
    var _a, _b;
    try {
        if (!isAdminOrSales(req)) {
            return res.status(403).json({ error: 'Admin or sales access required' });
        ...
        const existingProducts = await Product_1.default.find({}, 'sku name slug');
        const existingSkus = new Set(existingProducts.map(p => p.sku).filter((sku) => !!sku));
        const existingNames = new Set(existingProducts.map(p => p.name.toLowerCase()).filter((name) => !!name));
        const existingSlugs = new Set(existingProducts.map(p => p.slug).filter((slug) => !!slug));
        for (const row of productsData) {
            ...
            try {
                ...
                if (!sku || sku.trim() === '') {
                    ...
                    const existingSkusForCategory = await Product_1.default.find({
                        category: category,
                        sku: { $regex: `^${category.substring(0, 3).toUpperCase()}-` }
                    }).select('sku');
                    const skuList = existingSkusForCategory.map(p => p.sku).filter((s) => !!s);
                ...
                const imageUrls = [];
                if (row['Image URLs']) {
                    const urls = row['Image URLs'].split(';').map((url) => url.trim());
                ...
                const featured = ((_a = row['Featured']) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === 'yes' || ((_b = row['Featured']) === null || _b === void 0 ? void 0 : _b.toLowerCase()) === 'true';
                let supplierId = null;
                if (row['Supplier Name']) {
                    let supplier = await Supplier_1.default.findOne({
...
router.put('/bulk-update', auth_1.default, async (req, res) => {
    try {
        if (!isAdmin(req)) {
...
router.delete('/bulk-delete', auth_1.default, async (req, res) => {
    try {
        if (!isAdminOrSales(req)) {
            return res.status(403).json({ error: 'Admin or sales access required' });
        }
        ...
        const productsToDelete = await Product_1.default.find({ _id: { $in: validIds } });
        ...
        await Product_1.default.deleteMany({ _id: { $in: validIds } });
...
// ========== BULK ADJUST STOCK ==========
router.post('/bulk-adjust-stock', auth_1.default, async (req, res) => {
    try {
        if (!isAdminOrSales(req)) {
            return res.status(403).json({ error: 'Admin or sales access required' });
        }
...
router.get('/valuation', auth_1.default, async (req, res) => {
    try {
        if (!isAdminOrSales(req)) {
            return res.status(403).json({ error: 'Admin or sales access required' });
        }
